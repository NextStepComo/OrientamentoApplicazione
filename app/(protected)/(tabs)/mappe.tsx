...
export default function MappeScreen() {
  ...
  const handleMapMessage = (message: WebviewLeafletMessage) => {
    if (
      message.event === WebViewLeafletEvents.ON_MAP_MARKER_CLICKED &&
      message.payload?.mapMarkerID
    ) {
      const scuola = (listaScuole as any[]).find(s => s.id === message.payload!.mapMarkerID);
      if (scuola) {
  ...
  const handleSearch = (query: string) => {
    ...
    const results = (listaScuole as any[]).filter(s =>
      s.name.toLowerCase().includes(lower)
    );
    setSuggestions(results);
    setFilteredMarkers(results.map(s => ({
      id: s.id,
      position: s.position,
      title: s.name,
      icon: 'https://unpkg.com/leaflet@1.9.4/dist/images/marker-icon.png',
  ...
  return (
    <View style={{ flex: 1, paddingTop: insets.top }}>
              ...
              renderItem={({ item, index }) => (
                <TouchableOpacity
                  ...
                  className={`flex-row items-center gap-3 px-4 py-3 ${
                    index < suggestions.length - 1 ? "border-b border-[#CCDFFD]" : ""
                  }`}
                >
                  <MaterialIcons name="place" size={18} color="#066CF4" />
                  <Text className="flex-1 text-sm font-bold text-[#1A2433]" numberOfLines={1}>
                    {item.name}
                  </Text>
      ...
      {selectedScuola && (
        ...
        >
          {/* Immagine scuola */}
          {selectedScuola.image ? (
            <Image
              source={{ uri: selectedScuola.image }}
          ...
          <View className="p-4">
            ...
            <View className="flex-row items-start gap-3 mb-3">
              <View className="flex-1">
                ...
                <Text className="text-xs text-[#65758C] mt-0.5">
                  {selectedScuola.position.lat.toFixed(4)}, {selectedScuola.position.lng.toFixed(4)}
                </Text>
              </View>
            ...
            {selectedScuola.courses?.length > 0 && (
              <View className="mb-3">
                <Text className="text-xs font-black text-[#65758C] uppercase tracking-wide mb-2">
                  Indirizzi
                </Text>
                <ScrollView
                  horizontal
                ...
                >
                  {selectedScuola.courses.map((course: string, i: number) => (
                    <View
                      key={i}
                      className="mx-1 px-3 py-1.5 bg-[#E6F0FE] rounded-full border border-[#CCDFFD]"
                    >
            ...
            <TouchableOpacity
              activeOpacity={0.85}
              onPress={() => router.push({
                pathname: "/(protected)/(tabs)/scuole",
                params: { id: selectedScuola.id },
              })}
